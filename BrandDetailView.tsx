...
import { Brand, StaffTask, User, TaskStatus } from '../types';
...
const BrandDetailView: React.FC<BrandDetailViewProps> = ({ brand, tasks, users, currentUser, onBack, onEditTask, onAddComment, onReviewTask }) => {
  ...
  const stats = {
    ...
    pending: brandTasks.filter(t => t.status === 'Pending Approval').length,
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start gap-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full md:w-auto">
          {[
            ...
            { label: 'Pending Review', val: stats.pending, color: 'text-amber-500' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
        <div className="lg:col-span-1 space-y-6">
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-1">Personnel Assigned</h3>
          <div className="space-y-4">
            {teamMembers.length > 0 ? teamMembers.map(member => (
              ...
              <p className="text-xs font-medium text-slate-400 italic p-4">No team assigned yet.</p>
            )}
          </div>
        </div>

        <div className="lg:col-span-3 space-y-8">
          <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-1">Brand Protocol Queue</h3>
          <div className="space-y-6">
            {brandTasks.length === 0 ? (
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-[3rem] border-4 border-dashed border-slate-100 dark:border-white/5 py-20 text-center">
                <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No active protocols for this brand environment.</p>
              </div>
            ) : (
              brandTasks.map(task => (
                      ...
                      <div className="flex-grow">
                        ...
                        {task.status === 'Pending Approval' && isAdminOrLead && (
                          <div className="mt-6 flex gap-3">
                             <button 
                               onClick={() => onReviewTask(task.id, 'approve', 'Approved from brand detail view.')}
                               className="px-4 py-2 bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20"
                             >
                               Approve Completion
                             </button>
                             <button 
                               onClick={() => onReviewTask(task.id, 'reject', 'Revision required.')}
                               className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-rose-600 rounded-xl text-[9px] font-black uppercase tracking-widest border border-rose-100 dark:border-rose-900/20"
                             >
                               Request Revision
                             </button>
                          </div>
