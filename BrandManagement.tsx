...
import { Brand, ServiceType } from '../types';
...
const BrandManagement: React.FC<BrandManagementProps> = ({ brands, onCreateBrand, onUpdateBrand, onBrandClick }) => {
  ...
  const handleStartEdit = (e: React.MouseEvent, brand: Brand) => {
    e.stopPropagation(); // Prevent navigating to detail view when clicking edit
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="flex flex-col md:flex-row justify-between items-end gap-6">
        <div>
          <h2 className="text-4xl font-black text-slate-800 dark:text-white tracking-tight leading-none">Brand Management</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="space-y-4">
                <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Legal Brand Name</label>
                <input 
                  type="text" 
                  autoFocus
                  required
            ...
            <div className="flex flex-col sm:flex-row gap-6 pt-6">
              ...
              >
                {editingBrandId ? 'Commit Update to brand' : 'Provision Infrastructure & Generate Protocol'}
              </button>
            </div>
          </form>
        </div>
        ...
        {brands.map(brand => (
          ...
          >
            ...
            <div className="mt-12 pt-8 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
               ...
               <span className="text-[10px] font-black text-brand-blue dark:text-brand-cyan uppercase tracking-widest">Protocol Active</span>
            </div>
          </div>
        ))}
      </div>
