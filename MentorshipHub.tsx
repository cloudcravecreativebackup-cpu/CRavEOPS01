...
interface MentorshipHubProps {
  ...
  tasks: StaffTask[];
  currentUser: User;
  onClaimMentee: (menteeId: string) => void;
}

const MentorshipHub: React.FC<MentorshipHubProps> = ({ users, tasks, currentUser, onClaimMentee }) => {
  // Leads can recruit Staff Members and Mentees into their squad units only if they are unassigned
          ...
          return (
            <div 
              ...
              className={`bg-white dark:bg-slate-900 p-10 rounded-[3rem] border transition-all duration-300 relative group overflow-hidden ${
                isMySquad ? 'border-brand-blue/30 shadow-hard' : 'border-slate-200 dark:border-slate-800 shadow-soft'
              }`}
            >
              ...
              <div className="space-y-6 relative z-10">
                ...
                <div className="flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Squad Authority</span>
                  {mentor ? (
                    <span className={`text-[9px] font-black uppercase px-3 py-1.5 rounded-lg ${
                      isMySquad ? 'bg-brand-blue/10 text-brand-blue' : 'bg-slate-100 text-slate-500'
                    }`}>
                      {isMySquad ? 'Your Squad' : `Lead: ${mentor.name}`}
                    </span>
                  ) : (
                    <span className="text-[9px] font-black uppercase px-3 py-1.5 rounded-lg bg-amber-50 text-amber-600 animate-pulse">
                      Unassigned
                    </span>
                  )}
                </div>
                ...
                {!recruit.mentorId ? (
                  <button 
                    onClick={() => onClaimMentee(recruit.id)}
                  ...
                  <div className="w-full py-4 text-center border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl opacity-50">
                     <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Squad Occupied</span>
                  </div>
                )}
              </div>
              ...
              {isMySquad && (
                <div className="absolute top-6 right-6">
                   <div className="w-3 h-3 bg-brand-blue rounded-full shadow-[0_0_15px_rgba(29,78,216,0.6)]"></div>
                </div>
              )}
            </div>
          );
